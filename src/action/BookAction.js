import * as ActionType from './ActionType';
import BookApi from '../api/BookApi';

export const getResponse = data => ({
    type: ActionType.GET_BOOKS_RESPONSE,
    data
});


export function getAction() {
    return (dispatch) => {
        return BookApi.getAllDatas()
            .then(data => {
                dispatch(getResponse(data));
            }).catch(error => {
                throw error;
            });
    };
};

export const updateExistingResponse = () => ({
    type: ActionType.UPDATE_EXISTING_BOOK_RESPONSE
});

export const addNewResponse = () => ({
    type: ActionType.ADD_NEW_BOOK_RESPONSE
});


export function saveAction(BeingAddedOrEdited) {
    return function (dispatch) {
        return BookApi.saveData(BeingAddedOrEdited)
            .then(() => {
                if (BeingAddedOrEdited.id) {
                    dispatch(updateExistingResponse());
                } else {
                    dispatch(addNewResponse());
                }
            }).then(() => {
                dispatch(getAction());
            });
    };
}

export const getSingleResponse = data => ({
    type: ActionType.GET_SINGLE_RESPONSE,
    data: data
});

export function getSingleAction(Id) {
    return (dispatch) => {
        return BookApi.getData(Id)
            .then(data => {
                dispatch(getSingleResponse(data));
            }).catch(error => {
                throw error;
            });
    };
}

export const deleteResponse = () => ({
    type: ActionType.DELETE_BOOK_RESPONSE
});

export function deleteAction(Id) {
    return (dispatch) => {
        return BookApi.deleteData(Id)
            .then(() => {
                dispatch(deleteResponse());
            }).then(() => {
                dispatch(getAction());
            });
    };
}
